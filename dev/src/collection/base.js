protocor.require([
  'protocor.collection',
  'protocor.enviroment.object'
]);

protocor.collection.base = function () {
/* Variables */
	var _keys = [],
		_vals = [],
		_unique = false;
...
/* Implementation */
	...
	this.queue		= function (key, value, update) {
		update = protocor.argument(update, true);
	...
	this.unset 		= function (key) {
		var index = this.indexOf(key);
		if (index !== false) this.removeAt(index);
	...
	this.get 		= function (key, _default) {
        ...
        if (index === false && typeof(_default) == 'undefined') {
        	throw 'Key not found ' + key;
        } else if (index === false && typeof(_default) != 'undefined')
        	return _default;
	...
	this.insertAt 	= function (index, key, val) {
		...
		if (indexOld === false && this.count() > index && 0 <= index) {
			...
			throw 'Unable to insert, Key ' + key + ' already found at index ' + indexKey + ', or index out of bounds';
		}		
	...
	this.sort		= function (order) {
		order = protocor.argument(order, 'asc');
...
//						_iterations++;
	...
	this.reverse	= function () {
		...
		{
		    temporary = _keys[left]; _keys[left] = _keys[right]; _keys[right] = temporary;
		    temporary = _vals[left]; _vals[left] = _vals[right]; _vals[right] = temporary;
		}
	...
	this.shuffle	= function () {
		...
		while (--i > 0) {
			j = ~~ (Math.random() * (i + 1));
			t = _keys[j]; _keys[j] = _keys[i]; _keys[i] = t;
			t = _vals[j]; _vals[j] = _vals[i]; _vals[i] = t;
		}
	 ...
	 */
	this.each		= function (callback, results) {
		/**
		 * @var {Boolear} results
		 */
		...
		for(var _index in _keys) {
			...
			var _result = callback.apply(this, [_keys[_index], _vals[_index]]);
...
/* Common */
	this.toJSON 	= function () {
	    ...
	    for (var _index in _keys) {
	    	var k = _keys[_index];
	        var v = _vals[_index];
			if (v instanceof protocor.enviroment.object) {
				_ret[k] = v.toJSON();
			} else {
				_ret[k] = v;
			}
	...
	this.serialize 		= function () {
		...
		for (var _index in _keys) {
	        serialized['collection'][_keys[_index]] = protocor.serialize(_vals[_index]);
		}
	...
	this.unserialize	= function (serialized) {
		...
		if (serialized['collection']) {
			for(var key in serialized['collection']) {
		        ...
		        this.queue(key, protocor.unserialize(value));
		    }
		}
...
protocor.collection.base.prototype = new protocor.enviroment.object();
protocor.collection.base.prototype.constructor = protocor.enviroment.base;
protocor.register('protocor.collection.base', protocor.collection.base);
protocor.mode(protocor.collection, 'base', protocor.MODE_LOCKED);