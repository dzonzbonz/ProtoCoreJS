...
ProtoCore.Enviroment.LoaderInterface = function () {
...
ProtoCore.mode(ProtoCore.Enviroment, 'LoaderInterface', ProtoCore.MODE_LOCKED);

/**
 * @constructor
 * @extends {ProtoCore.Enviroment.LoaderInterface}
 */
ProtoCore.Enviroment.Loader = new ProtoCore.Enviroment.LoaderInterface();
ProtoCore.implement(ProtoCore.Enviroment.Loader, function () {
 ...
 */
    ...
    var _requireDependencies = function (jsDoc, dependancyStack, dependancyTree) {
        ...
        var isArray = ProtoCore.isArray(jsDoc);
        var isObject = ProtoCore.isObject(jsDoc);
...
//                });
        ...
        // if it is not a class then reqire the
            if (!_resolved['class']) {
                if (ProtoCore.isArray(_resolved)) {
...
//        ]);
        ...
        if (ProtoCore.isArray(required)) {
            loadStack = ProtoCore.clone(required);
        } else {
        ...
        ProtoCore.fork(_requireScript, loadStack, dependancyTree, (function (req, dt, cbd) {
    ...
    this.parse = function (required, callbackDone, callbackError) {
        ...
        _requirePath(required, 
            function (scriptContent) {
                ...
                ProtoCore.fork(_this.load, loadStack, callbackDone, callbackError);
...
ProtoCore.mode(ProtoCore.Enviroment, 'Loader', ProtoCore.MODE_LOCKED);